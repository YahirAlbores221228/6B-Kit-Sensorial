...
function Login() {
    ...
    const handleLoginClick = async (e) => {
        ...
        if (!email || !password) {
            Swal.fire({
                icon: "error",
                title: "Por favor, ingrese su correo electrónico y contraseña.",
                confirmButtonText: "Confirmar",
            });
            return;
        }
        try {
            const response = await fetch("https://apiweb.blubbysoft.site/users/log", {
                method: "POST",
                headers: {
            ...
            Swal.fire({
                icon: "error",
                title: "Error al realizar la solicitud. Por favor, inténtelo de nuevo.",
                confirmButtonText: "Confirmar",
            });
        }
    };
